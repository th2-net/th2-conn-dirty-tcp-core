...
class ProtoMessageBatcher(
    ...
    private inner class Batch : AutoCloseable {
        ...
        private fun send() = lock.withLock<Unit> {
            ...
            batch.build().runCatching(onBatch)
                .onFailure { LOGGER.error(it) { "Failed to publish batch: ${batch.toJson()}" } }
            batch.clearGroups()
            future.cancel(false)
        }
    ...
    companion object {
        ...
        private val GROUP_SELECTOR: (RawMessage.Builder) -> Any = { it.sessionGroup }
        private val ALIAS_SELECTOR: (RawMessage.Builder) -> Any = { it.sessionAlias to it.direction }
    }
}